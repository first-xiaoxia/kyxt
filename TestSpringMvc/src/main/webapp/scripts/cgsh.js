$(function(){
    $('#tab').datagrid({
        url:path+'/kycg/getKyjfShPageInfo',
        ...
        columns:[[
            ...
            {field:'shzt',title:"审核状态",width:100,align:'center',formatter:formatjfzt},
            ...
            {field:'cglsh',title:'操作',width:200,align:'center',
                formatter:function(value,rec,index){
                    console.log("aaaa"+index+","+rec+","+value);
                    var d = '<a href="#" mce_href="#" onclick="ckfj(\''+ value +'\')">查看附件</a> ';
                    return d;
                }
            }
        ]],
...
function ckfj(value) {
    ...
    $('#tab1').datagrid({
        ...
        showFooter:true,//是否显示行脚
        loadMsg :"加载数据中...",//加载数据时显示的提示消息
        queryParams: { cglsh: value }, //往后台传参数用的。
        //toolbar : '#tb',
        columns:[[
...
function shSave() {
    var shly = $("#bz").textbox('getValue');
    console.log(bz+rows.id);
    $.ajax({
        url:path+'/kycg/updatesh',
        method:'post',
        data:{
            cglsh:rows.cglsh,
            shly:shly,
